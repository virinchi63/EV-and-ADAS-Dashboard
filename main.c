...
int main(void)
{
	  ...
	  HAL_TIM_Base_Start(&htim2);          /*counter to count every 1us*/
	  ...
	  EV_Init(&ev);
	  HCSR04_Init();
	  ADAS_Init(&adas); //initialize F, L and R as 400cm by default
	  ...
	  while (1)
	  {
	      ...
	      if(sensor_flag == 1) /*100ms*/
	      {
	    	  sensor_flag=0;  /*read hsrc and print it every 100ms*/
	    	  HCSR04_ReadAll();

	    	  //update TTC, alarms, BSD
	    	  ADAS_Update( &adas, &ev);

	    	  //validating motor temp, soc and collision update false flag
	    	  Fault_Check(&flt, &ev, &adas);

	    	  //waiting for the data to rx and process the cmnd
	    	  Shell_Process();
